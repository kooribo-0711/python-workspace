...
'''
import zipfile, os, random, string, glob
...
'''
                               목적은 같으나 방식이 다른 두가지 방법
        비밀번호.encode()                         vs                      b'hiPassword'
            encode()                                                비밀번호가 고정일 때 사용
비밀번호가 번수(랜덤/입력값)일 때 사용 

소비자가 입력하거나 랜덤으로 출력되는                                고정적인 비밀번호니까 이미 비번 완성으로 판단    
비밀번호를 모두 확보한다음에 .encode()                              바로 비번을 컴퓨터가 이해하는 방식으로 변환
컴퓨터가 이해할 수 있는 글자 방식으로 변환
                                   
                                     둘 다 결과는 byte 형태로 변환
컴퓨터에게 소비자가 입력하거나 랜덤으로 입력되어                           컴퓨터에게 코드를 실행하며 
컴퓨터가 예측할 수 없는 데이터를 가져온다.                              미리 확보한 문자 비밀번호를 사용하겠다.

                                   

        glob.glob("*.png")                        vs                       if file.endswith(".png")
       오직 파일 찾기 전용                                                  파일 이외 폴더까지 모두 찾기
       if보다 빠르고 깔끔                                                   모두 섬세하게 찾는 만큼 glob보다 느림
'''